...
// •	Đề bài: Giả lập một hàm int read_temperature_sensor(int *temp_value) để đọc nhiệt độ từ một cảm biến. 
// Cảm biến này có thể gặp hai loại lỗi: chưa sẵn sàng để đọc, hoặc bị ngắt kết nối. 
// Hàm sẽ trả về trạng thái hoạt động, và nếu thành công, giá trị nhiệt độ sẽ được lưu vào biến temp_value.
// •	Yêu cầu:
...
// •	Dựa vào mã lỗi trả về, chương trình phải:
// o	In ra nhiệt độ nếu thành công.
// o	In thông báo "Sensor not ready, retrying..." nếu lỗi là -1.
// o	In thông báo "CRITICAL ERROR: Sensor disconnected! Halting system." và thoát khỏi vòng lặp nếu lỗi là -2.
// •	Sử dụng sleep() để mô phỏng thời gian chờ giữa các lần đọc.
#include<stdio.h>
#include<stdint.h>
#include<unistd.h>
#include<stdlib.h>
